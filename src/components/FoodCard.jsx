 ...
 *
 * 除了店名、分類、地圖連結以外，其他欄位都是選填：
 * 有照片就顯示照片，沒有就顯示分類圖示；
 * 評價、距離、介紹沒填就不會出現，卡片版面會自動收合。
 *
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] bg-surface p-2.5 ring-1 ring-black/[0.06] shadow-[0_1px_2px_rgba(16,24,40,0.04),0_14px_32px_-18px_rgba(16,24,40,0.12)] transition-[translate,box-shadow] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:shadow-[0_2px_4px_rgba(16,24,40,0.05),0_32px_54px_-24px_rgba(16,24,40,0.26)] focus-within:-translate-y-1.5">
      ...
      <div className="flex flex-1 flex-col px-3.5 pb-2.5 pt-5">
        {/* 沒有照片時，分類標籤改放在名稱上方 */}
        {!image && categoryInfo ? (
          <p className="mb-3 inline-flex w-fit items-center gap-1.5 rounded-full bg-forest/8 px-3 py-1.5 text-[0.7rem] font-medium tracking-[0.1em] text-forest">
            <span aria-hidden="true">{categoryInfo.emoji}</span>
            {categoryInfo.label}
          </p>
        ) : null}
        ...
        {/* mt-auto 讓按鈕貼齊卡片底部，同一列的卡片按鈕才會對齊 */}
        <div className="mt-auto pt-5">
          <a
            ...
            className="inline-flex min-h-11 w-fit items-center gap-2 rounded-full bg-forest/8 px-4 py-2.5 text-sm font-medium text-forest transition-[translate,background-color] duration-300 ease-out hover:-translate-y-0.5 hover:bg-forest hover:text-white"
          >
